                ...
                <main className="mx-auto max-w-7xl px-4 pt-8 sm:px-6 lg:px-8">
                    <div className="mb-10">
                        <nav className="mb-3 flex items-center gap-2 text-sm text-gray-400">
                            <Link href="/admin/dashboard" className="transition-colors hover:text-primary">Admin</Link>
                            <ChevronRight className="h-4 w-4" />
                            <span className="font-medium text-primary">Bookings</span>
                        </nav>
                        <h1 className="text-2xl font-bold tracking-tight text-primary">Requested Bookings</h1>
            ...
            <div className="flex justify-between items-start">
                <div>
                    <nav className="mb-3 flex items-center gap-2 text-sm text-gray-400">
                        <Link href="/admin/dashboard" className="transition-colors hover:text-primary">Admin</Link>
                        <ChevronRight className="h-4 w-4" />
                        <button onClick={() => setSelectedCategory(null)} className="transition-colors hover:text-primary">Requested Bookings</button>
                        <ChevronRight className="h-4 w-4" />
        ...
        <div className="fixed inset-0 bg-primary/80 backdrop-blur-md z-50 flex items-center justify-center p-6 lg:p-12 animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-5xl max-h-full rounded-[3rem] overflow-hidden flex flex-col shadow-2xl">
                ...
                <div className="flex-1 overflow-y-auto p-10 space-y-10">
                    {/* RENT-A-CAR: 4-section structured layout */}
                    {category === "rent-a-car" && (
                        <>
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                ...
                                <section>
                                    ...
                                    <div className="bg-gray-50 rounded-2xl p-6 space-y-4">
                                        <DetailItem label="Full Name" value={data.customerName} />
                                        <DetailItem label="Phone" value={data.phone} />
                                        <DetailItem label="Email" value={data.email} />
                                        <DetailItem label="NIC" value={data.nic} />
                                        <DetailItem label="License No." value={data.license} />
                                        <DetailItem label="Address" value={data.customerAddress} />
                                    </div>
                                </section>
                                ...
                                <section>
                                    ...
                                    <div className="bg-gray-50 rounded-2xl p-6 space-y-4">
                                        <DetailItem label="Vehicle" value={`${data.vehicle?.brand ?? ""} ${data.vehicle?.model ?? ""}`.trim()} />
                                        <DetailItem label="Plate Number" value={data.vehicle?.plateNumber} />
                                        <DetailItem
                                            label="Rental Period"
                                            value={`${new Date(data.rentalDate).toLocaleDateString()} → ${new Date(data.returnDate).toLocaleDateString()}`}
                                        />
                                        <DetailItem label="Total Fare" value={`LKR ${Number(data.totalFare).toLocaleString()}`} />
                                        <DetailItem
                                            label="Submitted"
                                            value={data.createdAt ? new Date(data.createdAt).toLocaleString() : "—"}
                                        />
                                        <DetailItem label="Terms Accepted" value={data.terms1 ? "Yes" : "No"} />
                                    </div>
                                </section>
                            </div>

                            {/* Section 3: Guarantor Information — full width */}
                            <section>
                                <h4 className="text-[10px] font-black text-red-600 uppercase tracking-[0.2em] mb-4 flex items-center gap-3">
                                    <User className="w-4 h-4" /> Guarantor Information
                                </h4>
                                <div className="bg-gray-50 rounded-2xl p-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
                                    <DetailItem label="Full Name" value={data.guaranteeFullname} />
                                    <DetailItem label="Phone" value={data.guaranteePhoneNo1} />
                                    <DetailItem label="NIC No." value={data.guaranteeNicNo} />
                                    <DetailItem label="Address" value={data.guaranteeAddress} />
                    ...
                    {/* ALL OTHER CATEGORIES: existing 2-section layout */}
                    {category !== "rent-a-car" && (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                            <section>
                                <h4 className="text-[10px] font-black text-red-600 uppercase tracking-[0.2em] mb-4 flex items-center gap-3">
                            ...
                            <section>
                                <h4 className="text-[10px] font-black text-red-600 uppercase tracking-[0.2em] mb-4 flex items-center gap-3">
                                    <Car className="w-4 h-4" /> Service Details
                                </h4>
                                <div className="bg-gray-50 rounded-2xl p-6 space-y-4">
                                    {category === "pickups" && (
                                        <>
                    ...
                    {category === "wedding" ? (
                        ...
                        <>
                            ...
                            <button 
                                onClick={onApprove} 
                                disabled={!selectedEmployeeId}
                                className="h-14 px-12 bg-primary text-white font-black rounded-3xl hover:bg-green-600 transition-all text-xs uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-primary"
                            >
                                Approve Booking
                            </button>
                        </>
...
function DocCard({ title, file, onOpen }: { title: string; file: string | null; onOpen: (url: string, title: string) => void }) {
    if (!file) return (
        <div className="space-y-4">
            ...
            <div className="aspect-4/3 bg-gray-100 rounded-4xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center gap-2 grayscale">
                <X />
                <span className="text-[9px] font-black text-gray-300">Not Uploaded</span>
            </div>
        </div>
    );
