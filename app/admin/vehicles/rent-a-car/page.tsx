...
export default function RentACarPage() {
    ...
    const fetchVehicles = async () => {
        setLoading(true);
        try {
            const result = await getVehiclesByCategory("Rent a Car");
            if (result.success) {
                setVehicles(result.data as any);
            }
        } catch (error) {
            console.error("Error fetching Rent-a-Car vehicles:", error);
        } finally {
            setLoading(false);
        }
    };
            ...
            <main className="mx-auto max-w-7xl px-4 pt-8 sm:px-6 lg:px-8">
                ...
                <div className="mb-8">
                    <nav className="mb-4 flex items-center gap-2 text-sm text-gray-400">
                        ...
                        <ChevronRight className="h-4 w-4" />
                        <span className="font-medium text-[#0f0f0f]">Rent a Car</span>
                    </nav>

                    <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <div className="flex items-center">
                                <h1 className="text-2xl font-black tracking-tight text-[#0f0f0f]">Rent a Car </h1>
                            </div>
                            <p className="mt-2 text-gray-500">Manage and oversee all vehicles assigned to the Rent-a-Car service.</p>
                        </div>

                        <Link
                            href="/admin/vehicles/add?category=Rent a Car"
