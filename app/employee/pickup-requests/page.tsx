...
import {
    MapPin, Navigation, Clock, Users, Briefcase,
    ChevronRight, CheckCircle2, XCircle, Loader2,
    Truck, RefreshCw, Filter, Phone, User
} from "lucide-react";
...
const STATUS_CONFIG: Record<string, { pill: string; icon: React.ReactNode; label: string }> = {
    ...
    COMPLETED: { pill: "bg-blue-100 text-blue-800",     icon: <CheckCircle2 className="w-3.5 h-3.5" />, label: "Completed" },
};
...
export default function EmployeePickupRequestsPage() {
    ...
    const fetchRequests = useCallback(async () => {
        setLoading(true);
        try {
            const res = await fetch(`/api/pickup/bookings?status=${filter}`);
    ...
    const handleAccept = async (id: number) => {
        ...
        setActionLoading(id);
        try {
            await fetch("/api/pickup/bookings", {
                method: "PATCH",
                ...
                <div className="mb-8">
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-2xl font-black tracking-tight text-[#0f0f0f]">Pickup Requests</h1>
                                <p className="text-sm text-gray-500">Review and manage customer pickup bookings.</p>
                            </div>
                        </div>
                        <button
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-32 bg-white rounded-[2.5rem] border border-dashed border-gray-200">
                        <div className="relative">
                            ...
                            <div className="absolute top-0 h-16 w-16 rounded-full border-4 border-emerald-500 border-t-transparent animate-spin" />
                        </div>
                            ...
                            return (
                                <div key={req.id} className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-6 hover:shadow-md transition-all">
                                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                                        ...
                                        <div className="flex-1 space-y-4">
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                ...
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm font-bold text-gray-900">{req.dropLocation}</p>
                                                    </div>
                                                </div>
                                                <div className="flex items-start gap-3">
                                                    <div className="p-2 bg-gray-50 rounded-lg flex-shrink-0">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm font-bold text-gray-900">
                                                            {req.pickupTime ? new Date(req.pickupTime).toLocaleString() : "—"}
                                                        </p>
                                                    </div>
                                                </div>
                                                <div className="flex items-start gap-3">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                <span className="inline-flex items-center gap-1 text-xs font-bold bg-gray-50 text-gray-600 px-3 py-1 rounded-full">
                                                    <Users className="h-3 w-3" /> {req.travelers} pax
                                                </span>
                                                <span className="inline-flex items-center gap-1 text-xs font-bold bg-gray-50 text-gray-600 px-3 py-1 rounded-full">
                                                    <Briefcase className="h-3 w-3" /> {req.luggageCount ?? 0} luggage
                                                </span>
                                                {req.isReturnTrip && (
                                                    <span className="inline-flex items-center gap-1 text-xs font-bold bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full">
                                                        ↩ Return trip
                                                    </span>
                                                )}
                                                <span className="inline-flex items-center gap-1 text-xs font-bold bg-gray-50 text-gray-600 px-3 py-1 rounded-full">
                                        ...
                                        {/* Right: price + actions */}
                                        <div className="flex flex-col gap-3 min-w-[180px] lg:items-end">
                                            <div className="bg-emerald-50 rounded-2xl p-4 text-center lg:text-right">
                                                <p className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">Total Fare</p>
                                                <p className="text-2xl font-black text-emerald-700">
                                                    LKR {parseFloat(req.price).toLocaleString()}
                                                </p>
                                            </div>
                                            ...
                                            {req.status === "PENDING" && (
                                                <div className="flex flex-col gap-2">
                                                    <button
                                                        ...
                                                        className="flex items-center justify-center gap-2 h-11 bg-emerald-500 hover:bg-emerald-600 text-white rounded-2xl text-sm font-black transition-all active:scale-95 disabled:opacity-50"
                                                    >
