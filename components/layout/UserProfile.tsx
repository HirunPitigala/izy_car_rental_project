        ...
        <div className="relative">
            ...
            >
                ...
                <div className="hidden text-left md:block">
                    <p className="text-sm font-bold text-[#0f0f0f] leading-tight">{session.user?.name || "User"}</p>
                    <p className="text-xs text-gray-500 capitalize leading-tight">{session.role}</p>
                </div>
                ...
                <>
                    ...
                    <div className="absolute right-0 z-40 mt-2 w-64 origin-top-right rounded-2xl bg-white p-1.5 shadow-premium border border-gray-100 focus:outline-none animate-in fade-in zoom-in duration-200">
                        <div className="px-4 py-4 mb-1 border-b border-gray-50">
                            <p className="text-sm font-bold text-[#0f0f0f]">{session.user?.name || "User"}</p>
                            <p className="truncate text-xs text-gray-500">{session.user?.email || "No email"}</p>
                            <div className="mt-2.5">
                                <span className="inline-flex items-center rounded-full bg-red-50 px-2.5 py-0.5 text-xs font-bold text-[#dc2626] capitalize">
                                    {session.role}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="mt-1 pt-1 border-t border-gray-50">
                            <button
                                ...
                                className="flex w-full items-center gap-2.5 px-3 py-2 text-sm font-medium text-red-600 rounded-lg hover:bg-red-50 transition-colors"
                            >
