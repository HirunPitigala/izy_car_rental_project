...
CREATE TABLE `pickup_requests` (
	...
	`assigned_employee_id` int,
	`created_at` datetime DEFAULT CURRENT_TIMESTAMP,
	CONSTRAINT `pickup_requests_pk` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `report` (
...
ALTER TABLE `pickup_requests` ADD CONSTRAINT `pickup_requests_customer_id_users_user_id_fk` FOREIGN KEY (`customer_id`) REFERENCES `users`(`user_id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `pickup_requests` ADD CONSTRAINT `pickup_requests_vehicle_id_vehicle_vehicle_id_fk` FOREIGN KEY (`vehicle_id`) REFERENCES `vehicle`(`vehicle_id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `pickup_requests` ADD CONSTRAINT `pickup_requests_driver_id_driver_driver_id_fk` FOREIGN KEY (`driver_id`) REFERENCES `driver`(`driver_id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `pickup_requests` ADD CONSTRAINT `pickup_requests_assigned_employee_id_employee_employee_id_fk` FOREIGN KEY (`assigned_employee_id`) REFERENCES `employee`(`employee_id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX `pr_customer_id_idx` ON `pickup_requests` (`customer_id`);--> statement-breakpoint
CREATE INDEX `pr_vehicle_id_idx` ON `pickup_requests` (`vehicle_id`);--> statement-breakpoint
CREATE INDEX `pr_assigned_employee_id_idx` ON `pickup_requests` (`assigned_employee_id`);--> statement-breakpoint
